...
def classify_shape(hull, points):
    # Calculate the aspect ratio of the convex hull
    ...
    # Check for regular polygons and circles
...
# Load data
data = pd.read_csv('occlusion2.csv')
...
dbscan = DBSCAN(eps=0.1, min_samples=10)  # Adjust these values based on your dataset
...
for cluster_id, color in zip(filtered_data['Cluster'].unique(), colors):
    ...
    if len(points) > 2:
        ...
        ax.add_patch(poly)
        ax.scatter(points[:, 0], points[:, 1], color=color, s=10)  # Plot points
