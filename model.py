...
#             label = labels[labels['ShapeID'] == shape_id]['is_rectangle'].values[0]
...
# # Save the model
# model.save('models/cnn_model_rectangle.h5')
...
model = tf.keras.models.load_model('models/cnn_model_rectangle.h5')
 
# Example usage
test_file_path = 'shapes/star_irregular_01.csv'
if os.path.exists(test_file_path):
    prediction = test_model(test_file_path, model)
    if prediction is not None:
        if prediction == 1:
            print(f'The shape in {test_file_path} is predicted to be a rectangle.')
        else:
            print(f'The shape in {test_file_path} is predicted to not be a rectangle.')
else:
