...
for file_name in os.listdir(data_dir):
    if file_name.endswith('.csv') and file_name != 'labels.csv':
        ...
        try:
            ...
            label = labels[labels['ShapeID'] == shape_id]['is_star'].values[0]
...
# Save the model
model.save('models/cnn_model_star.h5')
...
model = tf.keras.models.load_model('models/cnn_model_star.h5')

# Example usage
test_file_path = 'shapes/square_irregular_14.csv'
if os.path.exists(test_file_path):
    prediction = test_model(test_file_path, model)
    if prediction == 1:
        print(f'The shape in {test_file_path} is predicted to be a star.')
    else:
        print(f'The shape in {test_file_path} is predicted to not be a star.')
else:
